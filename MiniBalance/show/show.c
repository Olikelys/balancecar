...
void oled_show(void)
{
	int16_t temp=0,conv=0;
		//=============第一行显示小车模式=======================//	
	       ...
	       if(Flag_sudu==1)	OLED_ShowString(55,0,"Highspeed");
		     else             OLED_ShowString(55,0,"Lowspeed ");
	  //=============第二行显示温度和距离===============//	
	conv=Temperature&0x0000ffff;
	temp=(int16_t)(conv/34.0+365.3);
													OLED_ShowNumber(0,10,temp/10,2,12);
													OLED_ShowNumber(23,10,temp%10,1,12);
...
//			                    OLED_ShowString(105,10,"mm");
  	...
  	//=============第四行显示编码器2=======================//		
		...
		if(Angle_Balance<0)		
		{
			...
			OLED_ShowNumber(55,50,-Angle_Balance,3,12);
		}
		else					        
		{
			OLED_ShowString(50,50,"+"),
