...
//			if(Bi_zhang==1)Led_Flash(0);                                        //===LED闪烁;避障模式 指示灯常亮	
	    ...
	    Velocity_Pwm=velocity(Encoder_Left,Encoder_Right);                  //===速度环PID控制	 记住，速度反馈是正反馈，就是小车快的时候要慢下来就需要再跑快一点
 	    Turn_Pwm=turn(Encoder_Left,Encoder_Right,Gyro_Turn);            //===转向环PID控制     
   		...
   		Xianfu_Pwm();                                                       //===PWM限幅,和前后运动机械调整
			if(Pick_Up(Encoder_Left,Encoder_Right))//===检查是否小车被拿起
			Flag_Stop=1;	                                                      //===如果被拿起就关闭电机
			if(Put_Down(Angle_Balance,-Roll,Encoder_Left,Encoder_Right))              //===检查是否小车被放下
			Flag_Stop=0;	                                                      //===如果被放下就启动电机
      if(Turn_Off(Angle_Balance)==0)                              //===倾倒停车
...
{  
    static float Velocity,Encoder_Least,Encoder,Movement;
	  ...
	  if(Flag_sudu==2)  
		{
			Target_Velocity=15;                 //如果进入避障模式,自动进入低速模式
    }
		else
		{
  	...
  	if(1==Flag_Left||1==Flag_Right)                      //这一部分主要是根据旋转前的速度调整速度的起始速度，增加小车的适应性
		{
			...
			if(Turn_Convert<0.6)Turn_Convert=0.6;								//当前车速大，旋转增速设置小点
			if(Turn_Convert>3)Turn_Convert=3;										//当前车速小，旋转增速设置大点
		}																											//增加小车的稳定性，防止小车旋转过程中摔倒
	  else
		{
			Turn_Convert=0.9;
			Turn_Count=0;
...
//				USART_SendData(USART2 ,Angle_Balance); //发送单个数值
...
{ 		   
		...
		if(pitch_angle>(-5+ZHONGZHI)&&pitch_angle<(5+ZHONGZHI)&&roll_angle>-5&&roll_angle<5&&encoder_left==0&&encoder_right==0)         //倾角10以内，电机转速为0，持续2秒
		{
			count++;
			if (count==200)//保持2秒
			{
